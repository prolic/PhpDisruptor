...
class ConsumerRepositoryTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testShouldGetLastEventProcessorsInChain()
    {
        ...
        $this->consumerRepository->unMarkEventProcessorsAsEndOfChain($sequences);
        ...
        $this->assertTrue($this->eventProcessor1->getSequence() === $lastEventProcessorsInChain[0]);
    }

    public function testShouldRetrieveEventProcessorForHandler()
    {
        $this->consumerRepository->addEventProcessor($this->eventProcessor1, $this->handler1, $this->barrier1);
        $this->assertTrue($this->consumerRepository->getEventProcessorFor($this->handler1) === $this->eventProcessor1);
    }
    ...
    public function testShouldIterateAllEventProcessors()
    {
        ...
        foreach($this->consumerRepository->getConsumerInfos() as $info) {
            if (!$seen1
                && $info->getEventProcessor() === $this->eventProcessor1
                && $info->getHandler() === $this->handler1
            ) {
                $seen1 = true;
            } else if (!$seen2
                && $info->getEventProcessor() === $this->eventProcessor2
                && $info->getHandler() === $this->handler2
            ) {
                $seen2  = true;
            } else {
